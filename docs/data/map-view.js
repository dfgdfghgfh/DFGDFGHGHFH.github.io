// 设定集Wiki — 世界地图交互 v2（独立版）
function initMap(svgId, infoId, mapData, charData) {
    ...
    if(charData){
      ...
      if(chs.length>0){
        ...
        chs.forEach(function(ch){
          ...
          h+='<div style="background:'+C.surface+';border-left:3px solid '+sc2+';border-radius:0 6px 6px 0;padding:8px 10px;margin:4px 0;cursor:pointer;" onclick="selectChar(\''+ch.id+'\')">';
  ...
  function renderCharInfo(){
    ...
    if(ch.relationships&&ch.relationships.length>0){
      ...
      ch.relationships.forEach(function(r){
        ...
        h+='<strong>'+r.targetId+'</strong> <span style="color:'+C.gold+';font-size:11px;"> · '+r.type+'</span>';
